...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Tutorial.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Tutorial.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if(Tutorial.instance.settingsManager.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Tutorial.instance.clickGui.closeAllSettings();
				this.extended = b;
